...
export function solve(): string {
    ...
    let particles = _.map(input, (line, index) => parseLine(line, index));

    let s = 0;
    const probablyLongEnough = 10000;
    while (s < probablyLongEnough) {
        particles = _.map(particles, p => doStep(p));
...
function removeCollisions(particles: particle[]): particle[] {
    const grouped = _.groupBy(particles, p => p.position);
    const noCollisions = _.pickBy(grouped, pos => pos.length === 1);
    return _.flatten(_.values(noCollisions));
}
